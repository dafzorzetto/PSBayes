...
Gibbs_CASDMM<-function(c,matrix_X,T_var,P_obs,Y_obs){
  ...
  p_eta=c(-3,20)                  # mean and variance of normal distribution for eta param.
  # for Y-mode
  ...
  for (r in 1:R){
    ...
    M=t(P_tilde)%*%diag(1/rep((exp(lambda[1])),n0))%*%t(t(Y_obs_0))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde)%*%diag(1/rep((exp(lambda[1])),n0))%*%P_tilde+diag(rep(1/(p_theta[2]^2),2))
    ...
    M=t(P_tilde)%*%diag(1/(exp(lambda[1]+lambda[2]*P_obs_1)))%*%t(t(Y_obs_1))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde)%*%diag(1/(exp(lambda[1]+lambda[2]*P_obs_1)))%*%P_tilde+diag(rep(1/(p_theta[2]^2),4))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    ...
    a0=min(1, exp(sum(dnorm(c(Y_obs_0,Y_obs_1),mean_M,sqrt(var_star_0),log=T))-
                    sum(dnorm(c(Y_obs_0,Y_obs_1),mean_M,sqrt(var_0),log=T))))
    if (runif(1)<a0){
      lambda[1]=l0_star
      acc_0=acc_0+1
    } 
    ...
    a1=min(1, exp(sum(dnorm(Y_obs_1,mean_M[(n0+1):n],sqrt(var_star_1),log=T))-
                    sum(dnorm(Y_obs_1,mean_M[(n0+1):n],sqrt(var_0[(n0+1):n]),log=T))))
    if (runif(1)<a1){
      lambda[2]=l1_star
      acc_1=acc_1+1
    } 
    ...
    if(r>R_burnin){
      ...
      Y_1_imp=rnorm(n,cbind(1,P_1_r,P_0_r,P_0_r*P_1_r)%*%t(t(theta_1)),
                    sqrt(exp(lambda[1]+lambda[2]*P_1_r)))
  ...
  return(list(post_eta=post_eta, 
    ...
    S_all_cluster=S_all_cluster, 
    S_strata_cluster=S_strata_cluster$tab,
...
Gibbs_CASDMM_cov<-function(c,matrix_X,T_var,P_obs,Y_obs){
  ...
  for (r in 1:R){
    ...
    v_2=exp(lambda[1]+lambda[2]*P_obs_1)/((theta_1[3]+theta_1[4]*P_obs_1)^2)
    var_inv=1/sigma[p1]+1/v_2
    m_2=(Y_obs_1-theta_1[1]-theta_1[2]*P_obs_1-matrix_X[T1,]%*%theta_1[5:(4+n_X)])/(theta_1[3]+theta_1[4]*P_obs_1)
    P_mis[T1]=rnorm(T1,1/(var_inv)*(eta[p1]/sigma[p1]+m_2/v_2),1/(var_inv))
...
Gibbs_CASDMM_cov_P0<-function(c,matrix_X,T_var,P_obs,Y_obs){
  ...
  for (r in 1:R){
    ...
    # -----   THETA: means regression   -----
    # theta_0 = theta for treatment level 0
    P_tilde_0=cbind(1,matrix_X[T0,])
    Y_0_tilde=Y_obs_0-P_obs_0*theta_0[2]
    M=t(P_tilde_0)%*%diag(1/rep((exp(lambda[1]))^2,n0))%*%t(t(Y_0_tilde))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde_0)%*%diag(1/rep((exp(lambda[1]))^2,n0))%*%P_tilde_0+diag(rep(1/(p_theta[2]^2),1+n_X))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_0[-2]=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    
    # theta_1 = theta for treatment level 1
    P_tilde_1=cbind(1,P_obs_1,P_mis[T1]*P_obs_1,matrix_X[T1,])
    Y_1_tilde=Y_obs_1-P_mis[T1]*theta_1[3]
    M=t(P_tilde_1)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))^2))%*%t(t(Y_1_tilde))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde_1)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))^2))%*%P_tilde_1+diag(rep(1/(p_theta[2]^2),3+n_X))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_1[-3]=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    
    # theta common part (for covariates)
    P_tilde_com=c(P_obs_0,P_mis[T1])
    Y_com_tilde=c(Y_obs_0-P_tilde_0%*%theta_0[-2],Y_obs_1-P_tilde_1%*%theta_1[-3])
    phi_tilde=diag(1/c(rep((exp(lambda[1]))^2,n0),(exp(lambda[1]+lambda[2]*P_obs_1))^2))
    M=t(P_tilde_com)%*%phi_tilde%*%t(t(Y_com_tilde))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde_com)%*%phi_tilde%*%t(t(P_tilde_com))+(1/(p_theta[2]^2))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_com=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    ...
    mean_M=c(cbind(1,P_obs_0,matrix_X[T0,])%*%theta_0,cbind(P_tilde_1[,1:2],P_mis[T1],P_tilde_1[,3:(n_X+3)])%*%theta_1)
    var_0=c(rep(exp(lambda[1]),n0),exp(lambda[1]+lambda[2]*P_obs_1))
    var_star_0=c(rep(exp(l0_star),n0),exp(l0_star+lambda[2]*P_obs_1))
...
Gibbs_CASDMM_cov_conf<-function(c,matrix_X,T_var,P_obs,Y_obs, matrix_COV){
  ...
  sigma=rep(1,n_cluster)
  eta=runif(n_cluster,-10,10)
  theta_0=rep(0.1,2+n_C)
  theta_1=rep(0.1,4+n_C)
  lambda=rep(0.1,2)
  # cluster allocatio variables 
  xi_0=sample(1:n_cluster,n0,replace=TRUE)
  xi_1=sample(1:n_cluster,n1,replace=TRUE)
  ...
  cluster_allocation_1=matrix(NA,ncol=R-R_burnin,nrow=n)     
  post_beta=matrix(NA,ncol=R-R_burnin, nrow=length(beta_0)+length(beta_1))
  post_theta=matrix(NA,ncol=R-R_burnin,nrow=6+n_C*2)
  post_lambda=matrix(NA,ncol=R-R_burnin, nrow=2)
  ...
  for (r in 1:R){
    ...
    v_2=exp(lambda[1]+lambda[2]*P_obs_1)/((theta_1[3]+theta_1[4]*P_obs_1)^2)
    var_inv=1/sigma[p1]+1/v_2
    m_2=(Y_obs_1-theta_1[1]-theta_1[2]*P_obs_1-matrix_COV[T1,]%*%t(t(theta_1[5:(4+n_C)])))/(theta_1[3]+theta_1[4]*P_obs_1)
    P_mis[T1]=rnorm(T1,1/(var_inv)*(eta[p1]/sigma[p1]+m_2/v_2),1/(var_inv))
    ...
    var_inv=1/sigma[p1]+1/v_2
    m_2=(Y_obs_1-theta_1[1]-theta_1[2]*P_obs_1-matrix_COV[T1,]%*%t(t(theta_1[5:(4+n_C)])))/(theta_1[3]+theta_1[4]*P_obs_1)
    P_imp[T1]=rnorm(T1,1/(var_inv)*(eta[p1]/sigma[p1]+m_2/v_2),1/(var_inv))
    ...
    # -----   THETA: means regression   -----
    # theta_0 = theta for treatment level 0
    P_tilde=cbind(1,P_obs_0,matrix_COV[T0,])
    M=t(P_tilde)%*%diag(1/rep((exp(lambda[1])),n0))%*%t(t(Y_obs_0))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde)%*%diag(1/rep((exp(lambda[1])),n0))%*%P_tilde+diag(rep(1/(p_theta[2]^2),length(theta_0)))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_0=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    
    # theta_1 = theta for treatment level 1
    P_tilde=cbind(1,P_obs_1,P_mis[T1],P_mis[T1]*P_obs_1,matrix_COV[T1,])
    M=t(P_tilde)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))))%*%t(t(Y_obs_1))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))))%*%P_tilde+diag(rep(1/(p_theta[2]^2),length(theta_1)))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_1=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    ...
    mean_M=c(cbind(1,P_obs_0,matrix_COV[T0,])%*%theta_0,P_tilde%*%theta_1)
    var_0=c(rep(exp(lambda[1]),n0),exp(lambda[1]+lambda[2]*P_obs_1))
    var_star_0=c(rep(exp(l0_star),n0),exp(l0_star+lambda[2]*P_obs_1))
    ...
    if(r>R_burnin){
      ...
      P_0_r[T0]=P_imp[T0]
      P_0_r[T1]=P_mis[T1]
      Y_0_imp=rnorm(n,cbind(1,P_0_r,matrix_COV)%*%t(t(theta_0)),
                    sqrt(exp(lambda[1])))
      P_1_r=rep(NA,n)
      #P_1_r[T1]=P_obs_1
      P_1_r[T1]=P_imp[T1]
      P_1_r[T0]=P_mis[T0]
      Y_1_imp=rnorm(n,cbind(1,P_1_r,P_0_r,P_0_r*P_1_r,matrix_COV)%*%t(t(theta_1)),
                    sqrt(exp(lambda[1]+lambda[2]*P_1_r)))
...
Gibbs_CASDMM_cov_conf_P0<-function(c,matrix_X,T_var,P_obs,Y_obs, matrix_COV){
  ...
  for (r in 1:R){
    ...
    # -----   THETA: means regression   -----
    # theta_0 = theta for treatment level 0
    P_tilde_0=cbind(1,matrix_COV[T0,])
    Y_0_tilde=Y_obs_0-P_obs_0*theta_0[2]
    M=t(P_tilde_0)%*%diag(1/rep((exp(lambda[1]))^2,n0))%*%t(t(Y_0_tilde))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde_0)%*%diag(1/rep((exp(lambda[1]))^2,n0))%*%P_tilde_0+diag(rep(1/(p_theta[2]^2),1+n_C))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_0[-2]=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    
    # theta_1 = theta for treatment level 1
    P_tilde_1=cbind(1,P_obs_1,P_mis[T1]*P_obs_1,matrix_COV[T1,])
    Y_1_tilde=Y_obs_1-P_mis[T1]*theta_1[3]
    M=t(P_tilde_1)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))^2))%*%t(t(Y_1_tilde))+p_theta[1]/(p_theta[2]^2)
    V=t(P_tilde_1)%*%diag(1/((exp(lambda[1]+lambda[2]*P_obs_1))^2))%*%P_tilde_1+diag(rep(1/(p_theta[2]^2),3+n_C))
    V[lower.tri(V)] = t(V)[lower.tri(V)]
    theta_1[-3]=c(rmvnorm(1,solve(V)%*%M,solve(V)))
    ...
    mean_M=c(cbind(1,P_obs_0,matrix_COV[T0,])%*%theta_0,cbind(P_tilde_1[,1:2],P_mis[T1],P_tilde_1[,3:(n_C+3)])%*%theta_1)
    var_0=c(rep(exp(lambda[1]),n0),exp(lambda[1]+lambda[2]*P_obs_1))
    var_star_0=c(rep(exp(l0_star),n0),exp(l0_star+lambda[2]*P_obs_1))
