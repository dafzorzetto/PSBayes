...
SLM_GIbbs<-function(c_n,sim){
  ...
  # main variables
  T=sim[[c_n]]$data$T
  Y=T*sim[[c_n]]$data$Y_1+(1-T)*sim[[c_n]]$data$Y_0
  
  dD=matrix(rep(0,n*2),ncol=2)
  dD[T==1,2]=sim[[c_n]]$data$P_1[T==1]
  dD[T==0,1]=sim[[c_n]]$data$P_0[T==0]
  ...
  for (g in 2:R){
    ...
    #print(r)
    if (g%%500==0) print(paste0(g,"/",R," iterations"))
  ...
  Z_all_clusters=partition.BNPdens(list(clust=t(z_late_var[,(R_burnin+1):R])),dist = "VI")$partitions[1,]
  
  print(paste0("sample ",c_n, " done"))
  
  return(list(Z_all_clusters=Z_all_clusters,
    post_P_0_imp=apply(post_D_0[,(R_burnin+1):R],1,mean, na.rm=TRUE), 
    post_P_1_imp=apply(post_D_1[,(R_burnin+1):R],1,mean, na.rm=TRUE),
    post_Y_0_imp=apply(post_Y_0[,(R_burnin+1):R],1,mean, na.rm=TRUE), 
    post_Y_1_imp=apply(post_Y_1[,(R_burnin+1):R],1,mean, na.rm=TRUE)
  ))
